...
# Instagram bot that given a specific hashtag(#) iterates and likes through a list made of post urls.

class InstagramBot_likes:
    ...
    def darLike(self, hashtag):
        ...
        for i in range(1,1): #range(1,n) ->  n is the number of times the bot will simulate the scroll() action 
        ...
        for hrefsFotos in hrefsFotos:
            ...
            driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
            try:
                driver.find_element_by_xpath('/html/body/span/section/main/div/div/article/div[2]/section[1]/span[1]/button/span[@aria-label="Me gusta"]').click()  #Spanish
                time.sleep(1)
...
ig = InstagramBot_likes(usuario="[username]",contrasena="[password]")
ig.login()
